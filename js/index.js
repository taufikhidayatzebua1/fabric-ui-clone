...
function initLoader() {
  ...
  tl.to(
    ".loading-screen .icon-box svg g.tm",
    {
      opacity: 1,
      duration: 1.2,
      ease: Expo.easeInOut,
...
function pageTransitionOut() {
  ...
  if (document.querySelector("[data-transition-typewriter]")) {
    tl.fromTo(
      "[data-transition-typewriter] .single-char, [data-transition-typewriter] .single-word",
      {
        display: "none",
      },
      {
...
function initPageTransitions() {
  ...
  function initResetDataBefore() {
    ...
    $("[data-team-lightbox-status]").attr(
      "data-team-lightbox-status",
      "not-active"
    );
  }
  ...
  function initResetDataAfter() {
    ...
    $("[data-scrolling-transparant]").attr(
      "data-scrolling-transparant",
      "false"
    );
...
function initBasicFunctions() {
  ...
  $("[data-hover-scramble]").each(function () {
    let tlScrambleHover = gsap.timeline({
      paused: true,
    });
  ...
  $(".writing-grid").each(function () {
    ...
    gridWriting
      .find("[data-writing-card-status] a")
      .on("mouseenter", function () {
        gridWriting
          .find("[data-writing-card-status]")
          .attr("data-writing-card-status", "not-active");
  ...
  $("[data-team-card-id]").click(function () {
    ...
    if (
      $('[data-team-lightbox-id="' + teamCardID + '"]').attr(
        "data-team-lightbox-status"
      ) == "active"
    ) {
      $('[data-team-lightbox-id="' + teamCardID + '"]').attr(
      ...
      $('[data-team-lightbox-id="' + teamCardID + '"]')
        .attr("data-team-lightbox-status", "active")
        .siblings()
        .attr("data-team-lightbox-status", "transition");
      setTimeout(function () {
        $('[data-team-lightbox-id="' + teamCardID + '"]')
          .siblings()
          .attr("data-team-lightbox-status", "not-active");
      }, 600);
      gsap.from(
        $('[data-team-lightbox-id="' + teamCardID + '"]').find(".title"),
        {
          duration: 0.6,
          scrambleText: {
            text: "{original}",
  ...
  $('[data-invest-lightbox-toggle="toggle"]').click(function () {
    if (
      $("[data-invest-lightbox-status]").attr("data-invest-lightbox-status") ==
      "not-active"
    ) {
      $("[data-invest-lightbox-status]").attr(
        "data-invest-lightbox-status",
        "active"
      );
    } else {
      $("[data-invest-lightbox-status]").attr(
  ...
  $("[data-core-target]").on("mouseenter", function () {
    ...
    if (
      $('[data-core-id="' + coreID + '"]').attr("data-core-status") ==
      "not-active"
    ) {
...
function initSplitText() {
  ...
  var splitWordsWrap = new SplitText(".split-words-wrap", {
    type: "words",
    wordsClass: "single-word",
  });
  $(".split-words-wrap .single-word").wrapInner(
  ...
  var splitTextTypewriter = new SplitText(".split-typewriter", {
    type: "words, chars",
    wordsClass: "single-word",
    charsClass: "single-char",
  });

  var splitTextRead = new SplitText(".split-read", {
    type: "words",
    wordsClass: "single-word",
  });
}
...
function initCustomCursorV2() {
  ...
  $("[data-cursor-bubble-text]").on("mouseenter", function () {
    ...
    $(".custom-cursor")
      .find(".cursor-bubble")
      .attr("data-cursor-bubble-icon", $(this).data("cursor-bubble-icon"));
...
function initIsotopeFilter() {
  ...
  $('[data-single-filter-toggle="toggle"]').click(function () {
    if (
      $(this).parent().parent().attr("data-single-filter-status") ==
      "not-active"
    ) {
      ...
      $(this)
        ...
        .parent()
        .siblings()
        .find("[data-single-filter-status]")
        .attr("data-single-filter-status", "not-active");
...
function initMarqueeScroll() {
  ...
  $(".marquee-group").each(function (index) {
    ...
    var marqueeSpeed =
      marqueeGroup.find("[data-marquee-speed]").attr("data-marquee-speed") *
    ...
    const marqueeLeft = roll(
        marqueeGroup.find("[data-marquee-direction='left'] .marquee-content"),
        {
          duration: marqueeSpeed,
        }
      ),
      marqueeRight = roll(
        marqueeGroup.find("[data-marquee-direction='right'] .marquee-content"),
        {
          duration: marqueeSpeed,
        },
        true
      ...
      scroll = ScrollTrigger.create({
        ...
        onUpdate(self) {
          ...
          self.direction === -1
            ? marqueeGroup
                .find("[data-marquee-status]")
                .attr("data-marquee-status", "normal")
            : marqueeGroup
                .find("[data-marquee-status]")
  ...
  $("[data-vimeo-lightbox-target]").each(function (index) {
    ...
    vimeoLightboxOpenButton.click(function () {
      ...
      var vimeoLightboxContent =
        '<div class="overlay vimeo-append-content"><iframe src="https://player.vimeo.com/video/' +
        vimeoLightboxVideoID +
        '?api=1&background=1&autoplay=0&loop=0&muted=1&playsinline=1" width="640" height="360" frameborder="0" webkit-playsinline playsinline allow="autoplay;"></iframe><img class="overlay vimeo-overlay-placeholder" src="' +
        vimeoLightboxPlaceholder +
        '" /></div>';

      // Add or Replace Lightbox (Check if is new or current video)
      if (
        vimeoLightbox.attr("data-vimeo-lightbox-current-id") ==
        vimeoLightboxVideoID
      ) {
        // Nothing
      } else {
        ...
        vimeoLightbox
          .find(".single-vimeo-lightbox")
          .append(vimeoLightboxContent);
      }

      vimeoLightbox
        .find(".single-vimeo-lightbox")
        .css("--aspect-ratio", " " + vimeoLightboxRatio);
      ...
      var vimeoLightboxHeight = vimeoLightbox
        .find(".single-vimeo-calculate")
        .innerHeight();
      var vimeoLightboxWidth = vimeoLightbox
        .find(".single-vimeo-calculate")
        .innerWidth();
      var vimeoLightboxRatioNoPercent = parseInt(vimeoLightboxRatio) / 100;
      vimeoLightbox
        .find(".single-vimeo-calculate-wrap")
        .css(
          "width",
          " " +
            100 /
              ((vimeoLightboxWidth * vimeoLightboxRatioNoPercent) /
                vimeoLightboxHeight) +
            "%"
        );
      ...
      $(window).resize(function () {
        if (vimeoLightbox.attr("data-vimeo-status-activated") == "true") {
          vimeoLightboxHeight = vimeoLightbox
            .find(".single-vimeo-calculate")
            .innerHeight();
          vimeoLightboxWidth = vimeoLightbox
            .find(".single-vimeo-calculate")
            .innerWidth();
          vimeoLightbox
            .find(".single-vimeo-calculate-wrap")
      ...
      function vimeoLightboxClose() {
        ...
        vimeoLightbox.attr(
          "data-vimeo-lightbox-current-id",
          vimeoLightboxVideoID
        );
        ...
        val: function (e) {
            ...
            o = this[0];
          {
            ...
            if (o)
              return (
                (n =
                  st.valHooks[o.type] || st.valHooks[o.nodeName.toLowerCase()]),
          ...
          function y(e) {
            for (
              ...
              if ((n = C.relative[e[s].type])) c = [d(h(c), n)];
                ...
                if (
                  ((n = C.filter[e[s].type].apply(null, e[s].matches)), n[P])
                ) {
            ...
            return st.access(
              ...
              function (e) {
                return e === t
                  ? st.text(this)
                  : this.empty().append(
                      ((this[0] && this[0].ownerDocument) || V).createTextNode(
                        e
                      )
                    );
              },
          ...
          ((st.expr.filters.hidden = function (e) {
            return (
              ...
              (!st.support.reliableHiddenOffsets &&
                "none" ===
                  ((e.style && e.style.display) || st.css(e, "display")))
            );
          }),
          ...
          ajax: function (e, n) {
            function r(e, n, r, s) {
              ...
              2 !== x &&
                ...
                (y = t),
                c &&
                  h.trigger(l ? "ajaxSuccess" : "ajaxError", [w, p, l ? f : v]),
                m.fireWith(d, [w, N]),
                c &&
      ...
      (e.fn.attr2 = function () {
        ...
        (e.fn.ajaxSubmit = function (t) {
              ...
              if (!x.aborted && !F) {
                ...
                if ((M && M.location.href != m.iframeSrc) || T) {
                  ...
                  try {
                    if (T) throw "timeout";
                    var o =
                      "xml" == m.dataType || M.XMLDocument || e.isXMLDoc(M);
                    if (
                      (a("isXml=" + o),
                      !o &&
                        window.opera &&
                    ...
                    (x.responseText = u ? u.innerHTML : null),
                      ...
                      u &&
                        ((x.status =
                          Number(u.getAttribute("status")) || x.status),
                        (x.statusText =
                  ...
                  x.aborted && (a("upload aborted"), (i = null)),
                    ...
                    "success" === i
                      ? (m.success &&
                          m.success.call(m.context, E, "success", x),
          ...
          if (
            ...
            return (
              a("ajaxSubmit: submit aborted via beforeSubmit callback"), this
            );
